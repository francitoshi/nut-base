...
public class ActorSingle<M,R> implements ActorBase<M,R> , Runnable
{
    ...
    ActorSingle(Function<M,R> filter,ActorPool pool,BlockingQueue<Runnable> queue)
    {
        this.queue  = (queue!=null) ? queue :new LinkedBlockingQueue<>(QUEUE_SIZE);
        this.pool   = (pool!=null)? pool:ActorSingle.shared;
        this.filter = filter;
    }
    public ActorSingle(Function<M,R> filter,ActorPool pool)
    {
        this(filter,pool,null);
    }
    public ActorSingle(Function<M,R> filter)
    {
        this(filter,null);
    }
